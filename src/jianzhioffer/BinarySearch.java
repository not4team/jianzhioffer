...
public class BinarySearch {
	 ...
	 * (1)array[mid] > array[high]: 出现这种情况的array类似[x,x,x,6,x,x,2]，此时最小数字一定在mid的右边。 low = mid + 1
	 * (2)array[mid] == array[high]: 出现这种情况的array类似 [1,0,1,1,1]或者[1,1,1,0,1]，此时最小数字不好判断在mid左边 还是右边,这时只好一个一个试 ， low = low + 1 或者 high = high - 1 
	 * (3)array[mid] < array[high]:出现这种情况的array类似[x,x,x,4,x,x,6],此时最小数字一定就是array[mid]或者在mid的左 边。因为右边必然都是递增的。
	 * high = mid
	 * 
	...
	public static void main(String[] args) {
		int[] a = new int[] { 1, 3, 8, 15, 23 };
		BinarySearch b = new BinarySearch();
		System.out.println(b.binarySearch(a, 15));
	}
}
